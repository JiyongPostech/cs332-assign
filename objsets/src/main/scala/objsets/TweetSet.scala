...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      val accWithChildren = right.filterAcc(p, left.filterAcc(p, acc))
      if (p(elem)) accWithChildren.incl(elem)
      else accWithChildren
    }
    ...
    def mostRetweeted: Tweet = {
      var best = elem
      foreach(t => if (t.retweets > best.retweets) best = t)
      best
    }
